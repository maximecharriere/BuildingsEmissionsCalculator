...
ggplot(data_subset, aes(y = emissions_per_area)) +
  ...
  labs(title = "Boxplot with Outliers", y = "Emmission Intensity (kg CO2eq/m2)") +
  theme_minimal()
```
...
ggplot(data_subset, aes(x = emissions_per_area)) +
  ...
  labs(title = "Density Plot with Outliers", x = "Emission Intensity (kg CO2eq/m2)") +
  theme_minimal()
```
```{r, echo=FALSE}
# Density plot
ggplot(data_subset, aes(x = heat_energy)) +
  geom_density(fill = "lightblue", alpha = 0.5) +
  geom_rug(sides = "b", color = "red") +
  labs(title = "Density Plot with Outliers", x = "Energy Intensity (MJ/m2)") +
  theme_minimal()
```
...
# Energy Intensity in kWh/m2
conversion_factor <- 0.277777778 # from MJ to kWh
energy_intensity <- data_subset$heat_energy*conversion_factor
```
...
saveWorkbook(wb2, "Emission_Summary_Tables_Valiant.xlsx", overwrite = TRUE)
```
